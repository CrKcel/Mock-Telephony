...
public class IRadioNetworkImpl extends IRadioNetwork.Stub {
    ...
    private volatile IRadioNetworkResponse mRadioNetworkResponse;
    private volatile IRadioNetworkIndication mRadioNetworkIndication;
    ...
    public IRadioNetworkImpl(
        ...
        mMockModemConfigInterfaces[mSubId].registerForRadioStateChanged(
    ...
    private void notifyServiceStateChange() {
        ...
        Handler handler = mMockModemConfigInterfaces[mSubId].getMockModemConfigHandler();
        Message msg =
    ...
    private void updateNetworkStatus() {
        ...
        } else if (mServiceState.getIsHomeCamping()) {
            // Update to Home state
            mServiceState.updateServiceState(RegState.REG_HOME);
        } else if (mServiceState.getIsRoamingCamping()) {
    ...
    private boolean updateSimReady(AsyncResult ar) {
        ...
        for (int i = 0; i < numApplications; i++) {
            ...
            if (rilAppStatus.appState == android.hardware.radio.sim.AppStatus.APP_STATE_READY) {
                ...
                simPlmn =
                        mMockModemConfigInterfaces[mSubId].getSimInfo(
    ...
    public void setResponseFunctions(
        ...
        mService.registerRadioInterface(
                MockModemService.RADIO_INTERFACE_NETWORK, radioNetworkResponse.asBinder());
    }
    ...
    public void getUsageSetting(int serial) {
        ...
        RadioResponseInfo rsp = mService.makeSolRsp(serial, RadioError.REQUEST_NOT_SUPPORTED);
        try {
            // -1 is an explicitly-invalid UsageSetting; the framework ignores the
            // value whenever the response carries an error.
            mRadioNetworkResponse.getUsageSettingResponse(rsp, -1);
        } catch (RemoteException | NullPointerException ex) {
