...
public class MockModemMain {
    ...
    public static void main(String[] args) {
        ...
        // MockNetworkService defaults to the fixed no-service bootstrap state
        // (absent SIM, no camping) in its constructor, so early framework
        // registration queries observe it without an explicit publish. The
        // module daemon is intentionally not configurable: it only provides one
        // absent SIM slot and no network service as a stable fallback.
        ...
        if (!service.initialize(MockSimService.MOCK_SIM_PROFILE_ID_DEFAULT)) {
            throw new IllegalStateException("failed to initialize bootstrap SIM state");
        }
        System.out.println("mock modem initialized with static no-SIM bootstrap state");
